...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `${style.name} - Browse Designs`,
    description: style.description || `Browse millwork designs in the ${style.name} style.`,
  };
}

export default async function StyleBrowsePage({ params, searchParams }: Props) {
  const supabase = createServerSupabaseClient();
  ...
  // Get design IDs for this style
  const { data: styleDesigns } = await supabase
    .from('design_styles')
    .select('design_id')
    .eq('style_id', style.id);

  const designIds = styleDesigns?.map((ds) => ds.design_id) || [];
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex gap-1">
                {sortOptions.map((opt) => (
                  <Link
                    key={opt.value}
                    href={`/styles/${style.slug}?sort=${opt.value}${searchParams.price ? `&price=${searchParams.price}` : ''}`}
          ...
          <div className="flex gap-2 mb-6">
            {['all', 'free', 'paid'].map((p) => (
              <Link
                key={p}
                href={`/styles/${style.slug}?sort=${sort}${p !== 'all' ? `&price=${p}` : ''}`}
          ...
          <DesignGrid
            designs={designs || []}
            emptyMessage={`No designs in ${style.name} style yet. Check back soon!`}
          />
          ...
          {totalPages > 1 && (
            <div className="flex justify-center gap-2 mt-10">
              {page > 1 && (
                <Link
                  href={`/styles/${style.slug}?page=${page - 1}&sort=${sort}${searchParams.price ? `&price=${searchParams.price}` : ''}`}
