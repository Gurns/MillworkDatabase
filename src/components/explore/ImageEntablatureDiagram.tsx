...
// ─── Types ───
type EntablatureStyle = 'classical' | 'victorian' | 'craftsman' | 'gothic' | 'contemporary' | 'rustic';

interface ClickableLabel {
  /** Display text (shown on hover tooltip) */
  label: string;
  /** Category slug for navigation */
  slug: string;
  /** Percentage-based position within the cropped image */
  top: number;
  left: number;
  width: number;
  height: number;
...
// ─── Clickable element regions per style ───
// Positions are percentages of each cropped image (~460×367 top row, ~460×373 bottom row)

const STYLE_LABELS: Record<EntablatureStyle, ClickableLabel[]> = {
  classical: [
  ...
  contemporary: [
    // Right-side labels (most labels here)
    ...
    // Left-side labels
    { label: 'Minimalist Fascia Panel (Mullion)', slug: 'mullions-and-muntins', top: 42, left: 0, width: 38, height: 7 },
...
const STYLE_BUTTONS: { key: EntablatureStyle; label: string }[] = [
...
interface ImageEntablatureDiagramProps {
  /** Optional: function to build hrefs (for filter mode) */
  buildHref?: (slug: string) => string;
}

export function ImageEntablatureDiagram({ buildHref }: ImageEntablatureDiagramProps) {
  const [activeStyle, setActiveStyle] = useState<EntablatureStyle>('classical');
  const [hoveredLabel, setHoveredLabel] = useState<string | null>(null);

  const labels = STYLE_LABELS[activeStyle];
  const imageSrc = `/images/entablature/${activeStyle}.png`;
    ...
    <div>
      ...
      <div className="flex flex-wrap gap-1.5 mb-3">
        {STYLE_BUTTONS.map((s) => (
          <button
            key={s.key}
            onClick={() => { setActiveStyle(s.key); setHoveredLabel(null); }}
      ...
      <p className="text-[10px] text-gray-400 mt-1.5 text-center">
        Hover over any label to highlight · Click to browse designs
      </p>
    </div>
  );
}
